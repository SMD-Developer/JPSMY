...
//     }
     ...
     public function postLogin(Request $request)
    {
        ...
        if ($clientRegister && !$clientRegister->is_email_verified) {
            ...
            $message = 'Your email address is not verified. Please verify your email to login.';
            $actionMessage = $hasActiveOtp 
                ? 'Check your email for the verification code or click below to resend.'
                : 'Click below to receive a new verification code.';
...
//                     'mobileNumber'      => 'required|string|max:15|unique:client_register,mobileNumber',
...
//                 'state'       => $state_name, // Store only the state name
//                 'postal_code' => $request->postalCode,
//             ]);

//             // Insert into the client_register table
//             ClientRegisterModel::create([
//                 'client_id'         => $client->uuid, // Use the generated uuid
...
//                 'state_id'          => $state_id, // Store state ID
//                 'state'             => $state_name, // Store state name
//                 'district_id'       => $district_id, // Store district ID
//                 'district'          => $district_name, // Store district name
//                 'city'              => $request->city,
...
//         }
//     }
//     $primaryQuestions = DB::table('security_questions')->where('question_type', 'primary')
//     ->where('status', 1)
//     ->get();
//     $secondaryQuestions = DB::table('security_questions')->where('question_type', 'secondary')
//     ->where('status', 1)
//     ->get();
...
// }
    ...
    //         $hasValidOtp = OtpVerification::where('email', $email)
    //             ->where('type', 'registration')
    //             ->where('is_verified', false)
    //             ->where('expires_at', '>', now())
    //             ->exists();
    ...
    //         // Calculate remaining time for the OTP (for display purposes)
    ...
    //         $remainingTime = null;
    //         if ($latestOtp) {
    //             $remainingTime = now()->diffInMinutes($latestOtp->expires_at);
    //         }
    ...
    //         return view('clientarea.auth.otp-verification', compact('email', 'clientRegister', 'remainingTime'));
   ...
   public function showOtpVerification(Request $request)
        ...
        try {
            ...
            if ($latestOtp && $latestOtp->expires_at > now()) {
                ...
                if ($expiresAt->isFuture()) {
                    $remainingSeconds = $now->diffInSeconds($expiresAt, false); // false = signed difference
    ...
    //             // Mark email as verified in client_register table
    //             $clientRegister->update([
    //                 'is_email_verified' => true,
    //                 'email_verified_at' => now()
    //             ]);
    
    //             // Also update the main clients table if needed
    ...
    //             // Invalidate any other OTP records for this email
    ...
    //             ]);
                ...
                // // Send welcome/confirmation email (optional)
                // try {
