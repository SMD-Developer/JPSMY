    ...
    protected function trackAction($applicationId, $actionType, $request, $userType = 'admin')
{
   ...
   public function markAsRead(Request $request)
   {
    ...
    {         
            ...
            $query = Application::with(['state', 'landDistrict', 'landDivision', 'client', 'payment'])
            ...
            $query->where(function($q) use ($statusFilter) {
                ...
                } elseif ($statusFilter === 'in_review') {
                    $q->where('payment_status', 'in review');
                } else {
                    $q->whereNull('payment_status')
                    ->orWhereIn('payment_status', ['completed', 'in review']);
                }
            });
