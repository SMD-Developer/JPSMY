    ...
    attachClickEvent('#demo', function () {
        Swal.fire({
            ...
            if (result.isConfirmed && result.value) {
                ...
                Swal.fire('{{ __("app.application_rejected_successfully") }}', '', 'success').then(() => {
                    // Example AJAX request (Modify the URL to match your backend)
                    fetch('/reject-endpoint', {
                        method: 'POST',
                        headers: {
                            'Content-Type': 'application/json',
                            'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').getAttribute('content') // Ensure CSRF token is correctly included
                        },
                        body: JSON.stringify({ reason: rejectionReason })
                    }).then(response => response.json())
                      .then(data => {
                          console.log(data);
                          // Handle success (optional UI updates)
                      })
         ...
         attachClickEvent('#demo2', function () {
            Swal.fire({
                ...
                // text: '{{ __("app.do_you_want_to_send_to_finance") }}',
                icon: 'info',
                showCancelButton: true,
                confirmButtonColor: '#3085d6',
                cancelButtonColor: '#d33',
...
<script>
    document.addEventListener('DOMContentLoaded', function () {
        ...
        // Attach to 'Reject' button dynamically
        document.addEventListener('click', function (e) {
            if (e.target.matches('#rejectButton')) {
                const id = e.target.getAttribute('data-id'); // Get application ID
                
                Swal.fire({
                    title: "{{ __('app.reason_for_rejection') }}",
                    html: ` 
                        <label for="rejectionReason" style="display: block; text-align: center; font-weight: bold;">
                            {{ __('app.reason_for_rejection') }}
                        </label>
                        <textarea id="rejectionReason" class="swal2-textarea" style="width: 85%;" placeholder="{{ __('app.enter_reason_for_rejection') }}"></textarea>
                    `,
                    icon: 'warning',
                    showCancelButton: true,
                    confirmButtonColor: '#d33',
                    cancelButtonColor: '#3085d6',
                    confirmButtonText: "{{ __('app.yes_reject') }}",
                    cancelButtonText: "{{ __('app.cancel') }}",
                    preConfirm: () => {
                        const reason = document.getElementById('rejectionReason').value.trim();
                        if (!reason) {
                            Swal.showValidationMessage("{{ __('app.please_provide_rejection_reason') }}");
                            return false;
                        }
                        return reason;
                    }
                    ...
                    if (result.isConfirmed) {
                        ...
                        .then(data => {
                            if (data.success) {
                                Swal.fire("{{ __('app.application_rejected') }}", "{{ __('app.application_rejected_successfully') }}", 'success');
